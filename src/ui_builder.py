  ...
  <script>
    ...
    function renderCards() {
      ...
      sortedJobs.forEach((job, idx) => {
        ...
        card.innerHTML = `
          <!-- Top row: Company Logo + Category tag, Title + Location, Match Score -->
          <div class="flex items-start justify-between gap-3 border-b border-slate-800/80 pb-3 mb-3.5">
            <div class="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="mt-1 flex items-center gap-1.5 text-xs text-sky-400 font-medium">
                  <span>📍</span> <span>${loc}</span>
                </div>
              </div>
            </div>
            <div class="self-start shrink-0">
              <span class="px-2.5 py-1 rounded-full text-xs font-extrabold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
